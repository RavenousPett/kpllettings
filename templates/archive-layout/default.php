...
$sidebar_configs = apustheme_get_property_layout_configs();

?>
<!-- block-top-content -->
<?php
    if (apustheme_get_config('property_archive_show_top_content') && apustheme_get_config('property_archive_top_content')) {
        apustheme_display_block_content( apustheme_get_config('property_archive_top_content') );
    }
?>
<!-- end -->
<section id="main-container" class="main-content <?php echo apply_filters('apustheme_property_content_class', 'container');?> inner">

    <div class="main-content-top default clearfix">
        <?php apustheme_breadcrumbs(); ?>
        <header class="page-header">
            <?php the_archive_title( '<h1 class="page-title">', '</h1>' ); ?>
        </header><!-- .page-header -->
        <?php
            $display_mod = apustheme_woocommerce_get_display_mode();
            $item_style = apustheme_get_item_style();
            if($item_style == 'list') $display_mod = $item_style;
        ?>
