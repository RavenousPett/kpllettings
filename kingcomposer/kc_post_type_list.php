...
$readmore_text = !empty($readmore_text)? $readmore_text : esc_html__('Read more', 'preston');
...
if ( $the_query->have_posts() ) {
			...
			<div class="post-content">
				...
				<?php
					if ( has_post_format( array( 'chat', 'status' ) ) )
						$format_prefix = _x( '%1$s on %2$s', '1: post format name. 2: date', 'preston' );
					else
						$format_prefix = '%2$s';
					...
					if ( 'post' == get_post_type() && $show_author == 'yes' ) {
						printf( '<span class="author vcard"><i class="sl-user"></i> <a class="url fn n" href="%1$s" title="%2$s" rel="author">%3$s</a></span>',
							...
							esc_attr( sprintf( __( 'View all posts by %s', 'preston' ), get_the_author() ) ),
							get_the_author()
						);
					}
	...
	echo '<div '. implode(' ', $element_attribute) .'>';
	echo esc_html__('No posts found', 'preston');
	echo '</div>';
}
