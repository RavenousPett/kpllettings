	...
	if( !isset( $attachment_data[0] ) || empty( $attachment_data[0] ) ){
		...
		foreach($attachment_data as $i => $image){
			...
			switch( $click_action ){
				...
				case 'none':
					echo '<div class="item-grid grid-'. $columns .'"><img src="'. esc_attr($image[0]) .'" alt="" /></div>';
					break;

				case 'large_image':
					echo '<div class="item-grid grid-'. $columns .'"><a href="'. esc_attr( $attachment_data_full[$i][0] ) .'" target="_blank">'
						.'<img src="'. esc_attr($image[0]) .'" alt="" /></a></div>';
					break;

				case 'lightbox':
					echo '<div class="item-grid grid-'. $columns .'"><a class="kc-image-link kc-pretty-photo" data-lightbox="kc-lightbox"  href="'. esc_attr( esc_attr( $attachment_data_full[$i][0] ) ) .'">'
						.'<img src="'. esc_attr($image[0]) .'" alt=""/></a></div>';
					break;
