        ...
        function init_owl() {
            $(".owl-carousel[data-carousel=owl]").each( function(){
                ...
                if ($(this).data('autoplay')) {
                    ...
                    //config.stopOnHover = false;
                    config.loop = true;
                }
                if ($(this).data('autoplay_timeout')) {
                    config.autoplaySpeed = 1000;
                    config.autoplayTimeout = $(this).data( 'autoplay_timeout' );
                }
...
jQuery( document).ready( function($){
    ...
    if (typeof( $.prettyPhoto ) == 'object') {
        $(document).ready(function() {
            $(".popup-image, .popup-video").prettyPhoto();
        });
    }
    // mobile menu
    // mobile menu
    ...
    // header 2
    $(".header-v2 .action-menu").on('click', function(){
        $(this).toggleClass('active');
        $('.header-v2 .main-menu').toggleClass('active');
        $('.header-v2 .user-login').toggleClass('active');
        if ($(this).find('i').hasClass('mn-icon-103')) {
            $(this).find('i').addClass('mn-icon-4').removeClass('mn-icon-103');
        } else {
            $(this).find('i').addClass('mn-icon-103').removeClass('mn-icon-4');
        }
    } );
    // preload page
    var $body = $('body');
    ...
    $('.property-gallery-preview-owl').on('changed.owl.carousel', function(event) {
        setTimeout(function(){
            ...
            if ( $('.property-gallery-index .lite-carousel-play').length > 0 ) {
                $('.property-gallery-index li').eq(index).find('.thumb-link').addClass('active');
            } else {
...
(function($){
    ...
    $('.property-display-mod a').click(function(){
        var type = $(this).data('type');
        setCookie('apustheme_woo_mode', type, 30);
    });
})(jQuery)
...
function change_margin_top() {
    ...
    if (jQuery(window).width() >= 769) {
        if ( jQuery('#apus-header').length ) {
            var header_height = jQuery('#apus-header').height();
        }
    }
    if ( header_height > 0 ) {
        jQuery('#property-half-map').css({'top': header_height});
        jQuery('.half-map-content').css({'padding-top': header_height});
    }
}